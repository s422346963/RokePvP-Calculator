...
- `index.html` — 巨型单文件（约 190KB）：全部 UI 标记 + 全部计算逻辑都在行内 `<script>`（约 522 行起），入口为文件底部 `DOMContentLoaded`。
- `discus.html` — 讨论区页面（Giscus 评论）。
- `css/head.css` — 唯一样式表（主题变量、卡片、布局）。
- `data/*.js` — 游戏数据，均为全局常量，在 `index.html` 头部按序加载：
...
- 特性倍率：`calcAtkTraitMult` / `calcDefTraitMult` / `calcTraitMult` / `getTraitNeedsInput`（部分特性需要用户输入条件，见 `renderTraitCond`）。
...
- **不要凭记忆手写精灵/技能数值** —— 使用 `.codebuddy/skills/` 下的 AI 技能：
  - `sync-spirits`：按编号范围批量从 wiki 同步（配套 `scripts/extract_spirits.py`，需 beautifulsoup4）。
...
- `README.MD` 的项目结构一节已过时（`css/body.css`、`data/spirit_trait.js`、`data/spirit_skill.js` 已不存在），以本文件为准。
- `discus.html` 仍引用不存在的 `css/body.css`，为无害 404。
